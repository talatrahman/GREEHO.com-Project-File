	...
	<ul class="navbar-nav">
		...
		if( !empty( $dashboard_crm ) && houzez_check_role() ) {
			echo '<li class="nav-item dropdown">';
					echo '<a class="nav-link dropdown-toggle '.$ac_crm.'" data-toggle="dropdown" href="'.esc_url($dashboard_crm).'">
						<i class="houzez-icon icon-layout-dashboard mr-2"></i> '.houzez_option('dsh_board', 'My Properties').'
					</a>';
...
// 		}
		...
		if( !empty( $dashboard_listings ) && houzez_check_role() ) {
			echo '<li class="nav-item dropdown">
					<a class="nav-link dropdown-toggle '.esc_attr( $ac_props ).'" data-toggle="dropdown" href="'.esc_url($dashboard_listings).'">
